...
def generate_video_from_image(prompt, image_url, resolution, num_frames=81, fps=16, seed=None, 
    ...
    if model == "kling":
        ...
        try:
            # Use the fal_client for video generation - same pattern as WAN
            ...
            try:
                ...
                logger.info("Using synchronous FAL.ai call for Kling")
                result = fal_client.run(model_endpoint, arguments=request_params)
                ...
                for attempt in range(max_attempts):
                    ...
                    # Check status
                    status = fal_client.status(model_endpoint, request_id)
