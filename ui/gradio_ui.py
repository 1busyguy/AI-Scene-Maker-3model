    ...
    try:
        ...
        for chain in range(num_chains):
            ...
            try:
                ...
                # If not the last chain, extract frame and update current_image_url
                if chain < num_chains - 1:
                    ...
                    try:
                        # *** CRITICAL FIX: Use SIMPLE extraction without over-enhancement ***
                        best_frame_path, trimmed_video_path = video_processing.extract_simple_last_frame(
                            ...
                            avoid_over_enhancement=True  # CRITICAL: Avoid over-enhancement
                        )
                        
                        # CRITICAL FIX: Use the trimmed video for seamless transitions
                        if trimmed_video_path and os.path.exists(trimmed_video_path):
                            # Replace the untrimmed video with the trimmed version
                            video_paths[-1] = trimmed_video_path
                            logger.info(f"🔗 CHAIN {chain_number}: Using trimmed video for seamless transitions: {trimmed_video_path}")
                        else:
                        ...
                        try:
                            # *** THIS IS THE CRITICAL SECTION THAT MUST WORK ***
                            # Upload the extracted frame using standard upload (no over-enhancement)
                            new_image_url = fal_client.upload_file(best_frame_path, high_quality=False)
